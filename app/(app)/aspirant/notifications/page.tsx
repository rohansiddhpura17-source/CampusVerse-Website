...
export default function AspirantNotificationsPage() {
  ...
  const {
    ...
    queryKey: ['aspirantNotifications'],
  ...
  const markAsReadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['aspirantNotifications'] });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Admissions Alerts & Notices</h1>
            {unreadCount > 0 && (
          ...
          <p className="text-xs text-slate-500 mt-1">
            Stay updated on college application deadlines, newly cataloged scholarships, and counseling announcements.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className={`text-xs ${filter === 'ALL' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}`}
          >
            All Notifications
          </Button>
          <Button
            ...
            className={`text-xs ${filter === 'UNREAD' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}`}
          >
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                Admissions notifications and scholarship deadline reminders will appear here.
              </p>
            </div>
          ) : (
